<template>
  <div class="ui-text" @click="isClickText()">
    <div class="ui-text__caption" :class="{'ui-text__caption_active':modActive,
                   'ui-text__caption_completed':modCompleted}" v-if="dCaption">
      {{dCaption}}
    </div>

    <input class="ui-text__input" :class="{'ui-text__input_active':modActive}" type="text" :value="dValue" pattern="^\\d{10}$" ref="input" @focus="isActive()" @blur="isNotActive()" @input="isInputText()" :readonly=" dReadonly" />
    <hr class="ui-text__border" :class="{'ui-text__border_active':modActive}">
    <div class="ui-text__help ui-text__help_active" v-show="dHelp">{{dHelp}}</div>
  </div>
</template>
<script>
export default {
